...
## outlier_percents.txt
This file stores outlier data from multiple runs. Each run adds its own line, and each line has the name of the values column or scenario, the number of outliers detected, and the percent of the total predicted values that were outliers. The file's name is hardcoded in training_runner.py and is passed through a workflow to outliers.py

## outliers.txt
This file stores outlier data across multiple runs, and is edited or appended to when the outlier mode is 'complex' or 'both'. Each line has a complete sequence, the total number of times the sequence has been marked as an outlier, the name of each values column or scenario it was found as an outlier in, and whether it was overfitted or underfitted in the column. The file's name is hardcoded in training_runner.py and is passed through a workflow to outliers.py. 