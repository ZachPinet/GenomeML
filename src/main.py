...
def load_all_column_targets(columns_dir, reference_file, max_seqs):    
...
def main():
    ...
    file_num = 0  # 0-49
    file_num2 = 3
    do_single_column = False
    use_pca = False
    do_ensemble = False
    do_double_columns = True
    do_kfold = False
    transpose = False
    ...
    if use_pca:
        ...
        y_raw = load_all_column_targets(columns_dir, values_file, max_seqs)
    ...
    if do_double_columns:
        ...
        for i in range(23, 50):
            ...
            _, y2 = load_data(seq_file, values_file2, max_seqs=max_seqs)

            double_columns(
                x, y, y2, file_num, file_num2, train_percentage, show_bounds, 
                std_multiplier, frac, output_file, pct_file, mode
            )
